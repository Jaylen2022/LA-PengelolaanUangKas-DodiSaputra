...
function addpegawai($data) {
	...
	$query = mysqli_query($conn, "INSERT INTO pegawai VALUES ('', '$nama_pegawai', '$jenis_kelamin', '$unit_kerja', '$no_telepon', '$email')");
  	return mysqli_affected_rows($conn);
}
...
function editpegawai($data) {
	...
	$no_telepon = htmlspecialchars($data['no_telepon']);
	$email = htmlspecialchars($data['email']);
	$query = mysqli_query($conn, "UPDATE pegawai SET nama_pegawai = '$nama_pegawai', jenis_kelamin = '$jenis_kelamin', unit_kerja = '$unit_kerja', no_telepon = '$no_telepon', email = '$email' WHERE id_pegawai = '$id_pegawai'");
  	return mysqli_affected_rows($conn);
}
...
function addBulanPembayaran($data) {
	...
	if (mysqli_fetch_assoc($check_bulan)) {
		...
		$pegawai = "INSERT INTO uang_kas (id_pegawai, id_bulan_pembayaran, minggu_ke_1, minggu_ke_2, minggu_ke_3, minggu_ke_4) VALUES ";
		foreach ($datapegawai as $ds) {
		 	$pegawai .= "('{$ds['id_pegawai']}','{$id_bulan_pembayaran}','','','','')";
            $pegawai .= ",";
		}
		$pegawai = rtrim($pegawai,",");
		$query_pegawai = mysqli_query($conn, $pegawai);
	  	return mysqli_affected_rows($conn);
	}
}
...
function tambahpegawaiUangKas($data) {
	global $conn;
	$id_pegawai = htmlspecialchars($data['id_pegawai']);
	$id_bulan_pembayaran = htmlspecialchars($data['id_bulan_pembayaran']);
	$query = mysqli_query($conn, "INSERT INTO uang_kas VALUES ('', '$id_pegawai', '$id_bulan_pembayaran', '0', '0', '0', '0', '0')");
  	return mysqli_affected_rows($conn);
}
